...
# NEUTROSOPHIC CLOSING

nscl_image = ns_image.closing(ns_kernel)

# PLOT OF THE IMAGES

plt.suptitle("Neutrosophic closing (M=membership, I=indeterminacy, NM=non-membership)")
...
plt.subplot(4,3,7)
plt.imshow(nscl_image.getM(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("closing M")

plt.subplot(4,3,8)
plt.imshow(nscl_image.getI(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("closing I")

plt.subplot(4,3,9)
plt.imshow(nscl_image.getNM(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("closing NM")

plt.subplot(4,3,11)
plt.imshow(nscl_image.getRepresentation(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("closing")

plt.subplot(4,3,12)
plt.imshow(nscl_image.getRepresentation(binary=True, limit_value=0.04), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("binarized closing")
