...
# NEUTROSOPHIC OPENING

nsop_image = ns_image.opening(ns_kernel)

# PLOT OF THE IMAGES

plt.suptitle("Neutrosophic opening (M=membership, I=indeterminacy, NM=non-membership)")
...
plt.subplot(4,3,7)
plt.imshow(nsop_image.getM(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("opening M")

plt.subplot(4,3,8)
plt.imshow(nsop_image.getI(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("opening I")

plt.subplot(4,3,9)
plt.imshow(nsop_image.getNM(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("opening NM")

plt.subplot(4,3,11)
plt.imshow(nsop_image.getRepresentation(), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("opening")

plt.subplot(4,3,12)
plt.imshow(nsop_image.getRepresentation(binary=True, limit_value=0.04), cmap='gray')
plt.xticks([])
plt.yticks([])
plt.title("binarized opening")
